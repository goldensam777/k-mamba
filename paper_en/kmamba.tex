...
\usepackage{lmodern}  % Required for scalable fonts (fixes microtype error)
...
We introduce a \emph{reusable ND topological layer} that eliminates code
duplication between scan and convolution operators by providing common
primitives for ND indexing, stride calculation, and wavefront generation.
...
\begin{enumerate}
  ...
  \item \textbf{ND topological layer}: reusable primitives for ND indexing,
        stride calculation, and wavefront generation shared across
        \texttt{scan\_nd} and \texttt{convnd} (Section~\ref{sec:implementation}).
...
\subsection{ND Topological Layer}

A key architectural contribution is the separation of \emph{topological} operations from \emph{computational} ones. The ND topological layer (\texttt{km\_topology.h/c}) provides:

\begin{itemize}
  \item \textbf{Normalization}: \texttt{km\_normalize\_spatial\_topology()} validates and canonicalizes arbitrary dimension shapes.
  \item \textbf{Indexing}: \texttt{km\_make\_row\_major\_strides()}, \texttt{km\_unravel\_index()}, \texttt{km\_ravel\_index()} for O(1) ND coordinate conversion.
  \item \textbf{Wavefront generation}: \texttt{wavefront\_nd\_for\_level()} enumerates all positions at wavefront level $k$ where $\sum_i \text{idx}[i] = k$.
  \item \textbf{Reusable plans}: \texttt{KMWavefrontPlan} pre-computes and caches row-major offsets per level, enabling O(1) level access during scan/conv operations.
\end{itemize}

This layer eliminates code duplication between \texttt{scan\_nd.c} and \texttt{convnd.c}, which previously implemented their own stride/index helpers. Both now share the same topological primitives while retaining their distinct computational semantics (SSM monoid vs.
convolution kernel).
...
with four key contributions: (1) a formal proof that $d \times d$
wavefront scheduling achieves $d/2$ speedup, experimentally validated
to 0.1\% accuracy; (2) a reusable ND topological layer eliminating code
duplication between scan and convolution operators; (3) a Blelloch parallel
prefix scan reducing CUDA SSM depth from $O(L)$ to $O(\log L)$ with exact
same work; (4) a zero-dependency C/ASM/CUDA implementation of the full
training stack including MUONCLIP optimizer, all without Python or external
ML libraries.
...
% ─────────────────────────────────────────────────────────────────────────────
% Appendices come before references
\appendix
...
% ─────────────────────────────────────────────────────────────────────────────
% References (now after appendices)
