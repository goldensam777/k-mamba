...
/*
 * kmamba_hybrid — Instance k-mamba Hybrid CPU/GPU
 *
 * BPE Language Model basé sur k-mamba.
 * Config Hybrid : ~1.5M paramètres, embedding CPU, blocks GPU, logs CSV et corpus borné.
 *
 * Usage:
 *   ./kmamba_hybrid                          # entraîne sur texte intégré, puis génère
 *   ./kmamba_hybrid train <data.txt> [ckpt] [log-prefix]
 *   ./kmamba_hybrid gen   <ckpt> [prompt]    # génère depuis un checkpoint
 *   ./kmamba_hybrid chat  <ckpt>             # REPL interactif (chatbot)
 */
...
/* ============================================================
 * CONFIG — instance Hybrid (~1.5M params)
 * ============================================================ */
...
static KMambaConfig make_hybrid_config(void) {
    KMambaConfig cfg = {
...
static float evaluate_dataset(KMamba *m, const Dataset *ds, size_t max_windows) {
    ...
    for (size_t i = 0; i < windows; i++) {
        ...
        uint32_t *window = calloc(seq_tokens, sizeof(uint32_t));
...
static void generate(KMamba *m, const char *prompt, size_t gen_len) {
    ...
    uint32_t *ctx = calloc(L, sizeof(uint32_t));
    float *logits = calloc(L * V, sizeof(float));
...
int main(int argc, char *argv[]) {
    ...
    /* Création modèle — Hybrid: embedding/head CPU, blocks GPU */
    kmamba_backend_preference = KMAMBA_BACKEND_GPU; /* Blocks sur GPU */
    KMambaConfig cfg = make_hybrid_config();
    KMamba *m = NULL;
    ...
    char *owned_log_prefix = xstrdup_local(ckpt_path ? ckpt_path : "kmamba_hybrid");
    ...
    size_t seq_tokens = SEQ_LEN + 1;
    uint32_t *batch = calloc(BATCH_SIZE * seq_tokens, sizeof(uint32_t));
    ...
    if (total_tokens_target > 0) {
        ...
        /* Minimum 10 steps par epoch pour un entraînement utile */
        if (steps_per_epoch < 10) {
            steps_per_epoch = 10;
            int adjusted_epochs = (int)(total_steps / steps_per_epoch);
            if (adjusted_epochs < 1) adjusted_epochs = 1;
            printf("[Chinchilla] Ajustement: %d epochs (was %d) pour %zu steps/epoch\n",
                   adjusted_epochs, N_EPOCHS, steps_per_epoch);
        }
    ...
    for (int epoch = 1; epoch <= N_EPOCHS; epoch++) {
        ...
        for (size_t s = 0; s < steps_per_epoch; s++) {
            ...
            float loss = kmamba_train_batch_hybrid(m, batch, BATCH_SIZE);
            loss_sum += loss;
