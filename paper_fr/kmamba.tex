...
\author{
  ...
  \textit{Génie des Systèmes et IoT, IFRI--UAC, Bénin}\\
...
avec trois contributions clés : (1) une preuve formelle que l'ordonnancement
par front d'onde sur grille $d \times d$ atteint une accélération $d/2$,
validée expérimentalement à 0,1\% près ; (2) un scan préfixé parallèle de Blelloch
réduisant la profondeur CUDA du SSM de $O(L)$ à $O(\log L)$ avec le même travail ;
(3) une implémentation C/ASM/CUDA sans dépendance de la pile d'entraînement complète,
